...
Muskcoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
([`valgrind.supp`](https://github.com/muskcoin-project/muskcoin/blob/master/contrib/valgrind.supp))
...
To profile a running muskcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep muskcoind` -- sleep 60
```
...
- ThreadSocketHandler : Sends/Receives data from peers on port 2332.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 2331 for connections and services them.
