Muskcoin Core version 0.17.2 is now available from:

  <https://muskcoincore.org/bin/muskcoin-core-0.17.2/>
...
- A new document introduces Muskcoin Core's BIP174
  [Partially-Signed Muskcoin Transactions (PSBT)](https://github.com/muskcoin/muskcoin/blob/0.17/doc/psbt.md)
...
- When creating a transaction with a fee above -maxtxfee (default 0.1 TSLA), the RPC commands walletcreatefundedpsbt and fundrawtransaction will now fail instead of rounding down the fee. Be aware that the feeRate argument is specified in TSLA per 1,000 vbytes, not satoshi per vbyte. (#16639)
...
- #15012 Fix minor error in doc/psbt.md (muskcoinhodler)
