Muskcoin Core version 0.13.2 is now available from:

  <https://download.muskcoin.org/muskcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/muskcoin-project/muskcoin/issues>
...
updates will be released anymore. Without security updates, using a muskcoin
...
Muskcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Muskcoin Core 0.13.2 also introduces new default policy limits on the length and
...
caching. A sample config for apache2 could look like:
    ...
    <Location /muskcoinrpc>
        ProxyPass http://127.0.0.1:2331/
        ProxyPassReverse http://127.0.0.1:2331/
...
- Adding `dbcache=100` in muskcoin.conf
...
muskcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/muskcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Muskcoin Core code base has
...
- `muskcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `muskcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Muskcoin scripting system that in combination with
...
Muskcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Muskcoin Core.
...
The two phases are distinct in the Muskcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Muskcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Muskcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Muskcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of muskcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of muskcoins being spent is exactly
...
- **Increased security for multisig:** Muskcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Muskcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Muskcoin's current ledger (called the UTXO set),
  ...
  note that Muskcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Muskcoin users to individually opt-in to almost any change in the Muskcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Muskcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Muskcoin
client to Muskcoin Core 0.13.2 to signal for segwit support. When a
...
Since Muskcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `muskcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `muskcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
