...
const char * const MUSKCOIN_CONF_FILENAME = "muskcoin.conf";
const char * const MUSKCOIN_PID_FILENAME = "muskcoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "muskcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Muskcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Muskcoin
    // Mac: ~/Library/Application Support/Muskcoin
    // Unix: ~/.muskcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Muskcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Muskcoin";
#else
    // Unix
    return pathRet / ".muskcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", MUSKCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", MUSKCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Muskcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Muskcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Muskcoin Core developers";
    }
