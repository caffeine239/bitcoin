#!/usr/bin/env bash
# Copyright (c) 2016 The Muskcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   muskcoincore.org and muskcoin.org and compares them.
...
WORKINGDIR="/tmp/muskcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://muskcoincore.org"
HOST2="https://muskcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="muskcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'muskcoin-',
...
if [ $? -ne 0 ]; then
   echo "muskcoin.org failed to provide signature file, but muskcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "muskcoin.org and muskcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Muskcoin Core binary release signing key installed?"
   fi
